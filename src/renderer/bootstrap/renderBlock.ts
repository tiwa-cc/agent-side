...
export function renderBlock(block: Block, state: BlockRenderState): string {
  switch (block.type) {
    ...
    case "paragraph":
      return block.title
        ? `<section${classAttr(widthClass(block.width))}>${renderTitle(block.title, state.headingLevel)}<p>${renderInlineHtml(block.text)}</p></section>`
        : `<p${classAttr(widthClass(block.width))}>${renderInlineHtml(block.text)}</p>`;
    case "list": {
      const tag = block.ordered ? "ol" : "ul";
      return `${renderTitle(block.title, state.headingLevel)}<${tag}>${block.items.map((item) => `<li>${renderInlineHtml(item)}</li>`).join("")}</${tag}>`;
    }
    case "notice":
      return `<div class="${classNames("alert", toneClass[block.tone ?? "info"])}" role="note">${renderTitle(block.title, state.headingLevel, "h5")}<p>${renderInlineHtml(block.text ?? block.body ?? "")}</p></div>`;
    case "decision":
      return `<article class="border-start border-success border-4 ps-3 my-4">${renderTitle(block.title, state.headingLevel, "h5")}<p><strong>Decision:</strong> ${renderInlineHtml(block.decision)}</p>${block.rationale ? `<p><strong>Rationale:</strong> ${renderInlineHtml(block.rationale)}</p>` : ""}</article>`;
    case "risk":
      return `<article class="border-start border-warning border-4 ps-3 my-4">${renderTitle(block.title, state.headingLevel, "h5")}<p><strong>Risk:</strong> ${renderInlineHtml(block.risk)}</p>${block.mitigation ? `<p><strong>Mitigation:</strong> ${renderInlineHtml(block.mitigation)}</p>` : ""}</article>`;
    case "compare":
    ...
    case "include":
      return "";
    case "summary":
      return `<section>${renderTitle(block.title, state.headingLevel)}<p>${renderInlineHtml(block.body ?? "")}</p></section>`;
    case "points":
    case "constraint":
    case "assumption":
      return `${renderTitle(block.title, state.headingLevel)}<ul>${asArray(block.items).map((item) => `<li>${renderInlineHtml(item)}</li>`).join("")}</ul>`;
    case "keyValue":
      return `<section>${renderTitle(block.title, state.headingLevel)}<dl>${asArray(block.items)
        .map((item) => objectRecord(item))
        .map((item) => `<dt>${escape(stringify(item.key))}</dt><dd>${renderInlineHtml(item.value)}</dd>`)
        .join("")}</dl></section>`;
    case "openQuestion":
      return `<article class="border-start border-info border-4 ps-3 my-4">${renderTitle(block.title, state.headingLevel, "h5")}<p><strong>Question:</strong> ${renderInlineHtml(block.question ?? "")}</p>${block.context ? `<p><strong>Context:</strong> ${renderInlineHtml(block.context)}</p>` : ""}</article>`;
    case "command":
    ...
    case "todo":
      return `<section>${renderTitle(block.title, state.headingLevel)}<ul>${asArray(block.items)
        ...
        .join("")}</ul></section>`;
    case "issue":
      return `<article class="border-start border-danger border-4 ps-3 my-4">${renderTitle(block.title, state.headingLevel, "h5")}<p>${renderInlineHtml(block.body ?? "")}</p></article>`;
    case "checklist":
      return `<section>${renderTitle(block.title, state.headingLevel)}<ul class="list-unstyled">${asArray(block.items)
        .map((item) => objectRecord(item))
        .map((item) => `<li><input class="form-check-input me-2" type="checkbox" disabled${item.checked ? " checked" : ""}>${renderInlineHtml(item.label)}</li>`)
        .join("")}</ul></section>`;
    case "quote":
      return `<figure>${renderTitle(block.title, state.headingLevel)}<blockquote class="blockquote"><p>${renderInlineHtml(block.body ?? "")}</p></blockquote></figure>`;
    case "reference":
      return `<section>${renderTitle(block.title, state.headingLevel)}<ul>${asArray(block.items)
        .map((item) => objectRecord(item))
        .map((item) => {
